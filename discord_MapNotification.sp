...
public void OnPluginStart()
{
    ...
    g_cUsername = AutoExecConfig_CreateConVar("discord_map_notification_username", "NexxoN - SyStem", "Discord username");
    g_cColor = AutoExecConfig_CreateConVar("discord_map_notification_color", "#ff0000", "Hexcode of the color (with '#' !)");
...
public Action Timer_SendMessage(Handle timer)
{
    ...
    char sConnect[256];
    Format(sConnect, sizeof(sConnect), "steam://connect/nexxon.ddns.net:%d", iPort);

    /* Set bot avatar */
    char sThumb[256];
    Format(sThumb, sizeof(sThumb), "http://nexxon.ddns.net", sMap);
    ...
    {
        SetFailState("[NexxoN - SyStem] (Timer_SendMessage) Can't find webhook");
        return;
    }
    ...
    Embed.AddField("Jelenlegi map:", sMap, true);
    Embed.AddField("Online játékosok:", sPlayers, true);
    Embed.AddField("Gyors csatlakozás:", sConnect, true);
    hook.Embed(Embed);
    hook.Send();
    delete hook;
...
bool GetLastMap(char[] sMap, int iLength)
{
    ...
    {
        SetFailState("[NexxoN - SyStem] (GetLastMap) Cannot open file %s", FILE_LASTMAP);
        return;
    }
    delete fFile;
}
...
void UpdateLastMap(const char[] sMap)
{
    ...
    if (fFile != null)
    {
        ...
        {
            delete fFile;
            SetFailState("[NexxoN - SyStem] (UpdateLastMap) Cannot write file %s", FILE_LASTMAP);
            return;
        }
    }
    else
    {
        delete fFile;
        SetFailState("[NexxoN - SyStem] (UpdateLastMap) Cannot open file %s", FILE_LASTMAP);
        return;
    }
    delete fFile;
}
...
bool GetWebHook(const char[] sWebhook, char[] sUrl, int iLength)
{
    ...
    if (!FileExists(sFile))
    {
        SetFailState("[NexxoN - SyStem] (GetWebHook) \"%s\" not found!", sFile);
        delete kvWebhook;
        return false;
    }
    ...
    {
        SetFailState("[NexxoN - SyStem] (GetWebHook) Can't read: \"%s\"!", sFile);
        delete kvWebhook;
        return false;
    }
